...
class TaskServices:
    ...
    async def delete_soc_network(self, socnet_models, session):
    ...
    async def add_telegram_to_db(self, tg_list, session, task_id: int):
    ...
    async def add_instagram_to_db(self, insta_list, session, task_id: int):
    ...
    async def add_facebook_to_db(self, fb_list, session, task_id: int):
    ...
    async def GetAllTasks(self):
        ...
        return {"tasks": tasks}
    async def AddNewTask(self, task: TaskAdd, current_user: User = Depends(auth_services.get_current_user)):
        async with AsyncSessionLocal() as session:
            ...
            # Add lists to the db
            await self.add_telegram_to_db(telegrams, session=session, task_id=new_task.id)
            await self.add_instagram_to_db(instagrams, session=session, task_id=new_task.id)
            await self.add_facebook_to_db(facebooks, session=session, task_id=new_task.id)
    ...
    async def GetTaskByID(self, task_id: int, current_user: User = Depends(auth_services.get_current_user)):
        async with AsyncSessionLocal() as session:
    ...
    async def UpdateTaskByID(self, task_id: int, task_update: TaskUpdate, current_user: User = Depends(auth_services.get_current_user)):
        async with AsyncSessionLocal() as session:
            result = await session.execute(select(Task).filter_by(id=task_id, user_id=current_user.id))
            task = result.scalars().first()
    ...
    async def DeleteTaskByID(self, task_id: int, current_user: User = Depends(auth_services.get_current_user)):
        async with AsyncSessionLocal() as session:
            result = await session.execute(select(Task).filter_by(id=task_id, user_id=current_user.id))
            task = result.scalars().first()
            ...
            await self.delete_soc_network(telegrams.scalars().all(), session=session)
            await self.delete_soc_network(instagrams.scalars().all(), session=session)
            await self.delete_soc_network(facebooks.scalars().all(), session=session)
